...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  for (int r = 0; r < height; r++) {
    ...
    DMA[3].cnt = width | DMA_SOURCE_FIXED | DMA_ON; // write width bytes each row, source fix to write color,
  }
}

void drawFullScreenImageDMA(const u16 *image) {
  const u16 *yeet = image;
  for (int row = 0; row < 160; row++) {
    DMA[3].src = yeet;
    DMA[3].dst = &videoBuffer[OFFSET(row, 0, 240)];
    DMA[3].cnt = 240 | DMA_SOURCE_INCREMENT | DMA_ON;
    yeet += 240;
  }
}
